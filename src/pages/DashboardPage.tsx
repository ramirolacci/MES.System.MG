    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Vista general de producción</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-2">
            <p className="text-sm font-medium text-gray-600">Plan Total</p>
            <Target className="w-5 h-5 text-blue-600" />
          </div>
          <p className="text-3xl font-bold text-gray-900">{totalPlanned.toFixed(0)}</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          ...
          <p className="text-3xl font-bold text-gray-900">{totalProduced.toFixed(0)}</p>
          <p className="text-sm text-gray-500 mt-1">kg</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          ...
          <p className={`text-3xl font-bold ${compliance >= 100 ? 'text-green-600' : 'text-red-600'}`}>
            {compliance.toFixed(1)}%
          </p>
          <p className="text-sm text-gray-500 mt-1">del plan</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-6">Producción por Sector</h2>
                ...
                <div key={sector}>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className={`text-sm font-semibold ${
                      sectorCompliance >= 100 ? 'text-green-600' : 'text-red-600'
                    }`}>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-3">
                    <div
                      className={`h-3 rounded-full transition-all ${
                        sectorCompliance >= 100 ? 'bg-green-500' : 'bg-red-500'
                      }`}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-6">Top Productos</h2>
          <div className="space-y-4">
            {topProducts.map((item, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <div className="text-right">
                  <p className="text-xl font-bold text-gray-900">{item.produced.toFixed(0)} kg</p>
                  <p className={`text-sm font-semibold ${
                    item.compliance >= 100 ? 'text-green-600' : 'text-red-600'
                  }`}>
            ...
            {topProducts.length === 0 && (
              <p className="text-center text-gray-500 py-8">No hay datos de producción</p>
            )}
          </div>
        </div>
      </div>
